...
/*			_root.addNamespaceDeclaration("jnevents", NS_JNEVENTS);
	...
	public static IEvent createEvent(
		...
		el.addAttribute(new Attribute("id", Util.generateId()));
		el.addAttribute(new Attribute("hour", String.valueOf(parmTime.getHour())));
		el.addAttribute(new Attribute("min", String.valueOf(parmTime.getMinute())));
		el.appendChild(text);
		Day d = getDay(date);
		if (d == null)
			d = createDay(date);
	...
	//TASK 2-2 SMELL BETWEEN CLASSES Converted int mm and int hh params
    //to a Time object to address primitive obsession code smell.
	public static IEvent getEvent(CalendarDate date, Time parmTime) {
		Day d = getDay(date);
		if (d == null)
			return null;
		Elements els = d.getElement().getChildElements("event");
    ...
    //TASK 2-2 SMELL BETWEEN CLASSES Converted int mm and int hh params
    //to a Time object to address primitive obsession code smell.
	public static void removeEvent(CalendarDate date, Time parmTime) {
		Day d = getDay(date);
		if (d == null)
			d.getElement().removeChild(getEvent(date, parmTime).getContent());
	}
