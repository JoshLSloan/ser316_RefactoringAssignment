...
//    public static final String NS_JNNL = "http://www.openmechanics.org/2003/jnotes-noteslist";
    ...
    //Duplicate code - the identical nested loops in getAllNotes() and
    //getMarkedNotes() have been factored out into getNotes(boolean).
    ...
    /**
     * Returns a list of all notes. If marked paramter is true returns a list of
     * only notes that are marked.
     * @param marked If true return only notes that are marked otherwise all notes.
     * @return A vector of notes.
     */
    private Vector getNotes(boolean marked) {
        Vector v = new Vector();
            ...
            for (int mi = 0; mi < ms.size(); mi++) {
                ...
                for (int di = 0; di < ds.size(); di++) {
                    ...
                    for(int ni = 0; ni < ns.size(); ni++) {
                        NoteElement noteElement = (NoteElement) ns.get(ni);
                        INote note = new NoteImpl(noteElement.getElement(), _project);
