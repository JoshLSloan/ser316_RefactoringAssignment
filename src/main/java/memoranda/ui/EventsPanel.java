...
public class EventsPanel extends JPanel {
    ...
    void editEventB_actionPerformed(ActionEvent e) {
	    ...
	    //TASK 2-2 SMELL BETWEEN CLASSES Converted int mm and int hh params
	    //to a Time object to address primitive obsession code smell.
		...
		Time myTime = new Time(calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE));
        String text = dlg.textField.getText();

        if (dlg.noRepeatRB.isSelected()) {
            EventsManager.createEvent(CurrentDate.get(), myTime, text);
        }
        else {
            updateEvents(dlg, myTime, text);
        }    
	saveEvents();
    }
    ...
    void newEventB_actionPerformed(ActionEvent e, String tasktext, Date startDate, Date endDate) {
    	...
    	if (dlg.noRepeatRB.isSelected())
    		EventsManager.createEvent(eventCalendarDate, myTime, text);
    	else {
    		updateEvents(dlg, myTime, text);
    	}
    	saveEvents();
    ...
    private void updateEvents(EventDialog dlg, Time parmTime, String text) {
	int rtype;
        int period;
        ...
        //TASK 2-2 SMELL BETWEEN CLASSES Converted int mm and int hh params
        //to a Time object to address primitive obsession code smell.
        EventsManager.createRepeatableEvent(rtype, sd, ed, period, parmTime,
                text, dlg.workingDaysOnlyCB.isSelected());
    }
